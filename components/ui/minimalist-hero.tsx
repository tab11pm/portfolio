  ...
  return <section className={cn("relative flex min-h-screen w-full flex-col items-center justify-between overflow-hidden bg-[var(--bg)] px-6 py-8 font-sans text-[var(--text)] md:px-12", className)}>
    ...
    <div className="relative grid w-full max-w-7xl flex-1 grid-cols-1 items-center gap-8 py-10 md:grid-cols-3 md:gap-0">
      ...
      <div className="relative order-1 flex h-full min-h-80 items-center justify-center md:order-2">
        ...
        {imageFailed ? <div role="img" aria-label={imageAlt} className="relative z-10 h-80 w-56 bg-linear-to-br from-[#211c35] to-[#121217] shadow-[0_0_0_1px_#b7a5ff55] md:h-96 md:w-64" /> : <motion.div initial={reduceMotion ? false : { opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1], delay: reduceMotion ? 0 : 0.4 }} className="relative z-10 h-80 w-56 overflow-hidden md:h-96 md:w-64 lg:h-[28rem] lg:w-72"><Image src={imageSrc} alt={imageAlt} fill priority sizes="(min-width: 1024px) 18rem, (min-width: 768px) 16rem, 14rem" className="object-cover object-center" onError={() => setImageFailed(true)} /></motion.div>}
      </div>
