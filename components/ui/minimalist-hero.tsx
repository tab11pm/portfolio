  ...
  return <section className={cn("relative flex min-h-screen w-full flex-col items-center justify-between overflow-hidden bg-[var(--bg)] px-6 py-8 font-sans text-[var(--text)] md:px-12", className)}>
    ...
    <div className="relative grid w-full max-w-7xl flex-1 grid-cols-1 items-center gap-8 py-10 md:grid-cols-3 md:gap-0">
      ...
      <div className="relative order-1 flex h-full min-h-80 items-center justify-center md:order-2">
        ...
        {imageFailed ? <div role="img" aria-label={imageAlt} className="pointer-events-none relative z-40 h-80 w-56 bg-linear-to-br from-[#211c35] to-[#121217] shadow-[0_0_0_1px_#b7a5ff55] md:h-96 md:w-64 lg:h-[82vh] lg:w-[65vh]" /> : <motion.div data-testid="hero-portrait-frame" initial={reduceMotion ? false : { opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1], delay: reduceMotion ? 0 : 0.4 }} className="pointer-events-none relative z-40 h-80 w-56 overflow-visible md:h-96 md:w-64 lg:h-[82vh] lg:w-[65vh]"><Image src={imageSrc} alt={imageAlt} fill priority sizes="(min-width: 1024px) 65vh, (min-width: 768px) 16rem, 14rem" className="origin-[52%_42%] scale-[1.18] object-contain object-center lg:scale-[1.7]" onError={() => setImageFailed(true)} /></motion.div>}
      </div>
